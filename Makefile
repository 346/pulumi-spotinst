...
NODE_MODULE_NAME := @t346/pulumi-${PACK}
...
build::
	go install -ldflags "-X github.com/346/pulumi-${PACK}/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${TFGEN}
	go install -ldflags "-X github.com/346/pulumi-${PACK}/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}
	...
	cd ${PACKDIR}/nodejs/ && \
		...
		sed -i.bak "s/\@pulumi\/spotinst/@t346\/pulumi-spotinst/g" ./bin/package.json
	cd ${PACKDIR}/python/ && \
...
install::
	GOBIN=$(PULUMI_BIN) go install -ldflags "-X github.com/346/pulumi-${PACK}/pkg/version.Version=${VERSION}" ${PROJECT}/cmd/${PROVIDER}
